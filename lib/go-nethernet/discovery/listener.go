...
type Listener struct {
	...
	// pongDataV4 stores the legacy v4-format server data for older Bedrock clients.
	pongDataV4 atomic.Pointer[[]byte]
...
func (l *Listener) handleRequest(addr net.Addr) error {
	...
	if data := l.pongData.Load(); data != nil {
		if err := l.write(&ResponsePacket{ApplicationData: *data}, addr); err != nil {
			return fmt.Errorf("write v5 response: %w", err)
		}
		sent = true
	}
	if dataV4 := l.pongDataV4.Load(); dataV4 != nil {
		if err := l.write(&ResponsePacket{ApplicationData: *dataV4}, addr); err != nil {
			return fmt.Errorf("write v4 response: %w", err)
		}
		sent = true
	}
	if !sent {
...
// Both v4 (legacy) and v5 (1.21+) formats are stored for broad compatibility.
func (l *Listener) ServerData(d *ServerData) {
	...
	d4 := &serverDataV4{
		...
		GameType:       uint8(d.GameType),
		PlayerCount:    d.PlayerCount,
		MaxPlayerCount: d.MaxPlayerCount,
		EditorWorld:    d.EditorWorld,
		Hardcore:       d.Hardcore,
		TransportLayer: uint8(d.TransportLayer),
		ConnectionType: uint8(d.ConnectionType),
	}
	b4, err := d4.MarshalBinary()
	if err != nil {
		if l.conf.Log != nil {
			l.conf.Log.Error("error marshaling server data v4", slog.Any("error", err))
		}
		return
	}
	l.pongDataV4.Store(&b4)
}
...
// serverDataV4 is the legacy v4 ServerData binary format used by older Bedrock clients.
type serverDataV4 struct {
	ServerName     string
	LevelName      string
	GameType       uint8
	PlayerCount    int32
...
func (d *serverDataV4) MarshalBinary() ([]byte, error) {
	buf := &bytes.Buffer{}
	buf.WriteByte(4)
