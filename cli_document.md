...
CLI 支持两种联机协议，通过房间代码前缀或 `protocol` 参数区分：

| 协议 | 代码前缀 | 说明 | 游戏版本 |
|------|---------|------|---------|
| Scaffolding | `U/` | Java 版联机（默认） | Minecraft Java Edition |
| PaperConnect | `P/` | 基岩版联机 | Minecraft Bedrock Edition |

- `room.join` 根据代码前缀自动路由（`P/` → PaperConnect，其余 → Scaffolding）
- `room.create` 通过 `protocol` 参数指定协议，默认为 Scaffolding
- `room.stop`、`room.leave`、`room.cancel_join`、`room.status` 自动检测当前活跃的协议

#### `room.create`

创建联机房间（HOST 模式）。耗时约 2-5 秒。

**Scaffolding（Java 版，默认）：**
...
| `mc_port` | number | 否 | PaperConnect 固定使用 19132，此参数被忽略 |
...
加入房间（GUEST 模式）。根据房间代码前缀自动路由：`P/` → PaperConnect，其余 → Scaffolding。

Scaffolding 加入耗时约 5-30 秒，期间会推送 `progress` 响应。PaperConnect 加入耗时类似，无 progress 推送。
...
查询当前房间状态。根据角色和协议返回不同结构。PaperConnect 响应包含 `"protocol": "paperconnect"` 字段。
...
| `room.player_joined` | `PlayerInfo` | 仅房主模式：有新客人加入时推送（Scaffolding） |
| `room.player_left` | `PlayerInfo` | 仅房主模式：客人心跳超时时推送（Scaffolding） |
| `room.closed` | `{"reason": "..."}` | 仅房主模式：房间被关闭（Scaffolding） |
| `room.disconnected` | `{"reason": "..."}` | 仅客人模式：与房间断开连接（Scaffolding） |
| `room.guest_player_list_updated` | `PlayerInfo[]` | 仅客人模式：从房主同步到最新玩家列表时推送（Scaffolding） |
| `paperconnect.room.player_joined` | `PCPlayerInfo` | 新玩家加入房间（PaperConnect） |
| `paperconnect.room.player_left` | `PCPlayerInfo` | 玩家离开房间（PaperConnect，超时） |
| `paperconnect.room.closed` | `{"reason": "..."}` | 房间被关闭（PaperConnect） |
| `paperconnect.room.disconnected` | `{"reason": "..."}` | 与房间断开连接（PaperConnect） |
...
| `room.create` | `mc_port`, `player_name` | 2-5s | 创建房间（Scaffolding） |
| `room.create` | `player_name`, `protocol` | 2-5s | 创建房间（PaperConnect，protocol=`"paperconnect"`） |
| `room.stop` | 无 | <1s | 停止房间 |
| `room.join` | `code`, `player_name` | 5-30s | 加入房间（U/ 代码有 progress，P/ 代码无 progress） |
