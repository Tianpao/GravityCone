# GravityCone CLI SDK 文档

## 概述

GravityCone CLI 是一个独立的无 GUI 可执行文件，通过 **stdin/stdout** 与宿主进程通信，使用 JSON Lines 协议（每行一条 JSON 消息）。你可以从任何能启动子进程并读写其 stdio 的环境中调用它，例如 Electron、Tauri、Python、Node.js 或 Shell 脚本。

房间协议文档：

- [ScaffoldingMC（Minecraft Java Edition）](cli_scaffoldingmc.md)
- [PaperConnect（Minecraft Bedrock Edition）](cli_paperconnect.md)
...
| 协议 | 代码前缀 | 游戏版本 | 专属文档 |
|------|---------|----------|----------|
| ScaffoldingMC | `U/` | Minecraft Java Edition | [cli_scaffoldingmc.md](cli_scaffoldingmc.md) |
| PaperConnect | `P/` | Minecraft Bedrock Edition | [cli_paperconnect.md](cli_paperconnect.md) |

- `room.join` 根据代码前缀自动路由：`P/` 使用 PaperConnect，其他代码使用 ScaffoldingMC。
- `room.create` 通过 `protocol` 参数指定协议；省略时创建 ScaffoldingMC 房间。
- `room.stop`、`room.leave`、`room.cancel_join` 和 `room.status` 自动检测当前活跃的协议。
- `room.create`、`room.join`、`room.status` 的协议专属参数、结果和事件请参见对应文档。

### 中继节点参数（可选）

两种协议的 `room.create` / `room.join` 均支持可选参数 `relay`（对象）：

```json
{"id": 4, "method": "room.create", "params": {
  "mc_port": 25565,
  "player_name": "Steve",
...
| 字段 | 适用 | 说明 |
|------|------|------|
| `relay.node_id` | `room.create` | 中继节点 ID，编码进生成的房间码（0=自用中继，805=不使用公共节点）；省略时默认 0（`node_id` 也可传数字字符串） |
| `relay.url` | `room.create` / `room.join` | 中继节点连接地址，直接作为 EasyTier peer；房客端的地址获取由启动器自行处理 |

> CLI/FFI 不使用 Uptime 节点分发服务。不传 `relay` 或 `relay.url` 为空时仅使用内置节点：房主房间码编码 `805`（不使用公共节点），房客使用内置节点，双方在无公共中继的情况下依赖 P2P 直连。
...
添加节点地址，追加到启动参数中。下次创建或加入房间时生效。GravityCone Core 不会储存节点信息，调用方应自行储存。
...
| data 字段 | 类型 | 说明 |
|-----------|------|------|
| `udp_nat_type` | number | UDP NAT 类型：1=NoPAT(开放型互联网), 2=SymmetricFirewall(对称型防火墙), 3=FullCone(完全圆锥型NAT), 4=RestrictedCone(受限圆锥型NAT), 5=PortRestricted(端口受限圆锥型NAT), 6=SymmetricIncrement(对称型递增NAT), 7=Symmetric(对称型NAT) |
...
ScaffoldingMC 和 PaperConnect 的房间事件分别在其协议文档中定义。
...
const cli = spawn('./bin/gravitycone-cli', [
  '-p', 'tcp://1.2.3.4:5678', '-v', 'GC', '-m', 'PCL CE 联机房间',
...
| `room.create` | `mc_port`, `player_name`；可选 `relay` | 2-5s | 创建房间（ScaffoldingMC，详见 `cli_scaffoldingmc.md`） |
| `room.create` | `player_name`, `protocol`；可选 `relay` | 2-5s | 创建房间（PaperConnect，详见 `cli_paperconnect.md`） |
| `room.stop` | 无 | <1s | 停止当前协议的房间 |
| `room.join` | `code`, `player_name`；可选 `relay` | 5-30s | 加入房间（详见对应协议文档） |
