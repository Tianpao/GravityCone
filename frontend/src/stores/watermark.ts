...
export const useWatermarkStore = defineStore('watermark', () => {
  ...
  async function loadDemoImages() {
    ...
    error.value = ''
    try {
      const { ListDemoImages } = await import('@/../bindings/changeme/core/watermarkservice')
  ...
  async function encode(sourcePath: string, roomCode: string): Promise<WatermarkResult | null> {
    ...
    error.value = ''
    try {
      const { EncodeRoomCode } = await import('@/../bindings/changeme/core/watermarkservice')
  ...
  async function decode(imageBase64: string): Promise<string | null> {
    ...
    try {
      const { DecodeRoomCode } = await import('@/../bindings/changeme/core/watermarkservice')
